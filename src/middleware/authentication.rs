...
use actix_web::{dev::{self, Service, ServiceRequest, ServiceResponse, Transform}, web, Error, HttpMessage, FromRequest, HttpRequest, ResponseError};
    ...
    fn from_request(req: &HttpRequest, _: &mut Payload) -> Self::Future {
        ready(
            match req.extensions().get::<Identity>() {
                ...
                None => {
                    Err(ErrorInternalServerError("Not Found Identity"))
                }
            }
        )
    }
        ...
        S::Future: 'static,
{
    ...
    fn call(&self, req: ServiceRequest) -> Self::Future {
        let error_response = |req: ServiceRequest, status: StatusCode, message: String| -> Self::Future {
            Box::pin(async move {
                ...
                return match UserToken::find_by_token(conn, token.to_str().unwrap().to_string()) {
                    Ok(user_token) => {
                        if user_token.expire_time >= Local::now().naive_local() {
                            info!("token:{} 过期时间 {}",user_token.token, user_token.expire_time);
                            return error_response(req, StatusCode::RANGE_NOT_SATISFIABLE, "无效认证！请重新登录！".into());
                        }

                        match User::find(conn, user_token.user_id) {
                            Ok(user) => {
                                if user.locked_flag == User::LOCKED {
                                    return error_response(req, StatusCode::INTERNAL_SERVER_ERROR, "用户已被禁止登录！".into());
                                }
                            ...
                            Err(_) => {
                                error_response(req, StatusCode::INTERNAL_SERVER_ERROR, "无效用户！请重新登录！".into())
                            }
                        }
                    }
                    Err(_) => {
                        error_response(req, StatusCode::INTERNAL_SERVER_ERROR, "无效认证！请重新登录！".into())
                    }
                };
            }
        }
