...
pub fn save(conn: &mut PooledConn, user_id: i64, address_id: i64, cart_item_ids: Vec<i64>) -> result::Result<String> {
    let shopping_cart_items = super::shopping_cart::get_shopping_cart_items(conn, user_id, cart_item_ids)?;
    ...
    let goods_ids = shopping_cart_items.iter().map(|i| i.goods_id as u64).collect();
    ...
    if ShoppingCart::delete_by_cart_item_ids(conn, cart_item_ids)? > 0 {
        ...
        let order = Order::create(conn, NewOrder {
        ...
        OrderAddress::create(conn, OrderAddress {
            order_id: order.order_id,
...
pub fn list(conn: &mut PooledConn, filter: &Filter) -> result::Result<Page<OrderListResponse>> {
    ...
    let order_ids = orders_with_paginator.data.iter().map(|i| i.order_id).collect();

    let order_items = OrderItem::get_by_order_ids(conn, order_ids)?;

    let order_items_map = order_items.into_iter().into_group_map_by(|item| item.order_id);
...
pub fn cancel(conn: &mut PooledConn, order_no: String, user_id: i64) -> result::Result<()> {
    ...
    if order.order_status == 1 || order.order_status == -1 || order.order_status == -2 || order.order_status == -3 {
...
pub fn detail(conn: &mut PooledConn, order_no: String, user_id: i64) -> result::Result<(Order, Vec<OrderItem>)> {
    let order = Order::find_by_order_no(conn, order_no)?;
